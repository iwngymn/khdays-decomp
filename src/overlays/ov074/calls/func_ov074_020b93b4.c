/* Ov074_StepRiseState -- one frame of this boss's state 0x21 (the entry
...
extern struct BossCtx *data_ov074_020b9b80;                                    /* gpOv074Boss */
...
extern VecFx32 func_ov074_020b8b6c(struct Actor *pActor);                       /* Ov074_GetMarkPoint */
...
int func_ov074_020b93b4(struct Actor *pActor)
{
    VecFx32 vecDelta;
    VecFx32 vecStep;
    VecFx32 vecMark;
    ...
    pPhase = &data_ov074_020b9b80->phase;
    nRet = 0;
    if (func_02030788() == 0) {
    ...
    if (pPhase->nMode == MODE_NONE) {
        ...
        if (nKind == KIND_A || nKind == KIND_B || nKind == KIND_C) {
            if (pPhase->nHeight >= HEIGHT_LOW && pPhase->nHeight <= HEIGHT_HIGH) {
                ...
                vecMark = func_ov074_020b8b6c(pActor);
