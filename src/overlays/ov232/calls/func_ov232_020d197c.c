/* func_ov232_020d197c -- AI hover-dive step (byte-identical in ov232 /
...
extern const Vec3 data_ov232_020d368c;                                 /* base heading */
extern const Vec3 data_02042258;                                       /* kVecForward */
extern void  func_ov232_020d06b4(Vec3 *pOut, DiveNode *pNode, const Vec3 *pIn); /* rotate by the owner yaw */
extern void  func_ov232_020d0620(DiveNode *pNode);                      /* update the heading */
...
extern int   func_ov232_020d0748(DiveNode *pNode, const Vec3 *pDir, int nArg); /* probe for a hit */
...
void func_ov232_020d197c(DiveNode *pNode)
{
    Vec3 vDir;
    Vec3 vAxis;
    Vec3 vForward;
    ...
    pState = pNode->pState;
    vDir = data_ov232_020d368c;
    bDone = 0;
    func_ov232_020d06b4(&vDir, pNode, &vDir);
    pState->vVel = vDir;
    pState->nTimer += pNode->pClock[0x2c / 4];
    switch (pState->nPhase) {
    case 0:
        func_ov232_020d0620(pNode);
    ...
    if (func_ov232_020d0748(pNode, &vDir, 0) != 0) {
        pState->nHits++;
        if (pState->nHits == 1) {
