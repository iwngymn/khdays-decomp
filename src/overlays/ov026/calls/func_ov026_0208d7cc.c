/* func_ov026_0208d7cc -- Ov008_BuildShopList: collect the shop records listed
...
extern int  func_ov026_0208d614(int nSlot, int nCategory, u32 nIndex);   /* Ov008_IsShopRecordShown */
extern void NNSi_FndFreeFromDefaultHeap(void *pBlock);
extern void *NNSi_FndAllocFromDefaultExpHeap(u32 nSize);                /* AllocDefault */
extern u32  func_ov026_0208cc74(int nIndex);                            /* Ov008_ComputeRecordPrice */

Ov008ParamRecord **func_ov026_0208d7cc(int nSlot, int nCategory, int *pCount)
{
    int nListed = 0;
    u32 i;
    int nOut;
    ...
    for (i = 0; i < data_ov026_0209136c->nShopCount; i++) {
        if (func_ov026_0208d614(nSlot, nCategory, i) != 0) {
            nListed++;
        }
    }
    if (nListed == 0) {
    ...
    data_ov026_0209136c->apSorted = NNSi_FndAllocFromDefaultExpHeap(nListed * sizeof(Ov008ParamRecord *));
    nOut = 0;
    for (i = 0; i < data_ov026_0209136c->nShopCount; i++) {
        if (func_ov026_0208d614(nSlot, nCategory, i) != 0) {
            pRecord = &data_ov026_0209136c->pShopRecords[i];
            data_ov026_0209136c->apSorted[nOut] = pRecord;
            pRecord->nPrice = pRecord->nBasePrice;
            if (pRecord->nDivisor != 0 && pRecord->nExponent != 0 && pRecord->nMaxPrice != 0) {
                pRecord->nPrice = func_ov026_0208cc74(nOut);
