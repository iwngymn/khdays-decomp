/* func_ov026_0208d614 -- Ov008_IsShopRecordShown: whether shop record nIndex
...
extern int func_ov026_0208d40c(int nItemId);                            /* Ov008_GetItemParamWord */
extern int func_ov026_0208d4f0(Ov008ParamRecord *pRecord, int nSlot, int nCategory, u32 nIndex); /* Ov008_IsShopRecordListed */

int func_ov026_0208d614(int nSlot, int nCategory, u32 nIndex)
{
    ...
    pRecords = data_ov026_0209136c->pShopRecords;
    pRecord = &pRecords[nIndex];
    pNext = nIndex < data_ov026_0209136c->nShopCount - 1 ? &pRecords[nIndex + 1] : 0;
    pPrev = (int)nIndex > 0 ? &pRecords[nIndex - 1] : 0;
    nShown = 0;
    if (func_ov026_0208d40c(pRecord->pItemDef->nItemId) != 0) {
        bPassB = 0;
        bPassA = 0;
        if (func_ov026_0208d4f0(pRecord, nSlot, nCategory, nIndex) != 0) {
            bOk = 1;
            bNextHidden = 1;
            if (nIndex != data_ov026_0209136c->nShopCount - 1
            ...
            if (!bNextHidden && func_ov026_0208d4f0(pNext, nSlot, nCategory, nIndex) != 0) {
                bOk = 0;
            }
            if (bOk) {
                bPassA = 1;
