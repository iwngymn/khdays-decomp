/* func_ov026_02086af4 -- Ov008_DrawCounterPanel: redraw the shop's counter
...
extern int   func_ov026_0208e658(int nItemId);                           /* special item? */
extern void *func_ov026_02083530(void *pTracker, int nTag);              /* ov008_FindEntryByTag */
extern void  func_ov026_020835c4(void *pTracker, void *pEntry);          /* Ov008_TagTracker_InvokeCallback */
extern void  func_0203011c(void *pSurface, int nX, int nY, int nW, int nH); /* Obj_InvokeInnerVtable8: clear */
extern Ov008MenuEntry *func_ov026_0208427c(void *pManager, int nId);     /* FindEntryById */
extern void  func_ov026_020843e8(void *pManager, Ov008MenuEntry *pEntry, int bVisible); /* SetEntrySlotsVisible */
extern void  func_ov026_02084488(void *pManager, Ov008MenuEntry *pEntry, int nFrame); /* Ov008_ReleaseTwoSlotsEx */
extern void  func_ov026_02085bf4(void *pSurface, void *pText, int nX, int nY, int nStyle, int nShadow); /* Ov008_DrawStringShadowed */
extern void  func_02032710(int hSlots, int nCell, int bVisible);         /* Slot_SetVisible */
extern void  func_ov026_02086778(u32 nValue, int *pCells);               /* Ov008_DrawThreeDigitCounter */

void func_ov026_02086af4(Ov008ParamRecord *pRecord, int nCount)
{
    ...
    if (pRecord == 0) {
        ...
        bSpecial = func_ov026_0208e658(pRecord->pItemDef->nItemId);
    }
    if (pRecord == 0) {
        nOwned = 0;
    } else {
    ...
    if (pPanel->bReady == 0) {
        func_ov026_020835c4(ctx->tagTracker, func_ov026_02083530(ctx->tagTracker, TAG_COUNTER));
        func_0203011c(pSurface, 0, 0, 0xb0, 0x30);
        pPanel->bReady = 1;
    }
    for (i = WIDGET_FIRST; i <= WIDGET_LAST; i++) {
        func_ov026_020843e8(pWidgets, func_ov026_0208427c(pWidgets, i), 0);
    }
    if (pRecord != 0) {
        func_ov026_020843e8(pWidgets, func_ov026_0208427c(pWidgets, table.aWidget[bSpecial != 0 ? 3 : pRecord->pItemDef->nCategory]), 1);
    }
    func_0203011c(pSurface, 0, 0, 0xb0, 0x30);
    if (pRecord != 0) {
        func_ov026_02085bf4(pSurface, pRecord->pItemDef->pName, 0xd, 0xb, 4, 8);
    }
    ...
    if (pRecord != 0) {
        ...
        if (bSpecial == 0) {
            func_ov026_02086778(nOwned, pPanel->aDigit[0]);
        }
    }
    pMark = func_ov026_0208427c(pWidgets, WIDGET_MARK);
    pUp = func_ov026_0208427c(pWidgets, WIDGET_UP);
    pDown = func_ov026_0208427c(pWidgets, WIDGET_DOWN);
    pQty = func_ov026_0208427c(pWidgets, WIDGET_QTY);
    if (bSpecial == 0 && nOwned != nCount && nCount >= 0) {
        func_ov026_020843e8(pWidgets, pQty, 0);
        func_ov026_020843e8(pWidgets, pMark, 1);
        if (nOwned < nCount) {
            if (!pDown->bLocked) {
                func_ov026_020843e8(pWidgets, pDown, 1);
                func_ov026_02084488(pWidgets, pDown, 0);
            }
            func_ov026_020843e8(pWidgets, pUp, 0);
        } else {
            if (!pUp->bLocked) {
                func_ov026_020843e8(pWidgets, pUp, 1);
                func_ov026_02084488(pWidgets, pUp, 0);
            }
            func_ov026_020843e8(pWidgets, pDown, 0);
        }
        func_ov026_02086778(nCount, (&pPanel->aDigit[1])[ctx->nTab != 1]);
    } else {
        func_ov026_020843e8(pWidgets, pUp, 0);
        func_ov026_020843e8(pWidgets, pDown, 0);
        func_ov026_020843e8(pWidgets, pMark, 0);
        bShow = 0;
        bTab = ctx->nTab == 0 || ctx->nTab == 2;
        if (bTab && nOwned == nCount) {
            bShow = 1;
        }
        func_ov026_020843e8(pWidgets, pQty, bShow);
    }
}
