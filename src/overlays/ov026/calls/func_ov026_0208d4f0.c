/* func_ov026_0208d4f0 -- Ov008_IsShopRecordListed: whether shop record pRecord
 ...
 * (data_ov026_020911d0[nSlot]); day 0 and the final day (0x165) are only listed
...
int func_ov026_0208d4f0(Ov008ParamRecord *pRecord, int nSlot, int nCategory)
{
    u32 nDay;
    u32 nCap;
    int nPrereq;
    ...
    nDay = pRecord->nDay;
    nListed = 0;
    nCap = data_ov026_020911d0[nSlot];
    bCategoryOk = 0;
    bTierOk = 0;
    bDayOk = 0;
    if (nDay <= nCap) {
        if ((nDay != 0 && nDay != DAY_FINAL) || data_ov026_0209136c->bShowAll != 0) {
            bDayOk = 1;
        }
    }
    if (bDayOk) {
