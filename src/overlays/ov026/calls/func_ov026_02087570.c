/* func_ov026_02087570 -- Ov008_RebuildShopList: rebuild the shop's record
...
extern Ov008ParamRecord **func_ov026_0208d7cc(int nLevel, int nSlot, u32 *pCount); /* Ov008_BuildShopList */
extern Ov008ParamRecord **func_ov026_0208d970(int nSlot, u32 *pCount);           /* item list */
extern Ov008ParamRecord **func_ov026_0208dd34(int nCategory, u32 *pCount);       /* Ov008_BuildRecipeList */
extern Ov008ParamRecord **func_ov026_0208de40(int nList, u32 *pCount);           /* reward list */
extern u32  func_ov026_0208e218(Ov008ParamRecord **apRecords, int nIndex, u32 bSet); /* Ov008_QueryRecordUnlock */
extern void func_ov026_020874b4(void);                            /* refresh the cursor */
extern void func_ov026_0208737c(u32 nCount);                      /* publish the count */

int func_ov026_02087570(int bKeep, int bForce)
{
    ...
    pShop = &ctx->shop;
    nCount = 0;
    slots = *(Ov008CategorySlots *)data_ov026_02091110;
    recipes = *(Ov008RecipeCategories *)&data_ov026_020910b8[0x16];
    rewards = *(Ov008RewardLists *)&data_ov026_020910b8[0];
    switch (ctx->nTab) {
    case TAB_SHOP:
        ...
        apRows = func_ov026_0208d7cc(nLevel, slots.aSlot[pShop->nCategory], &nCount);
        break;
    case TAB_ITEMS:
        apRows = func_ov026_0208d970(slots.aSlot[pShop->nCategory], &nCount);
        break;
    case TAB_RECIPES:
        apRows = func_ov026_0208dd34(recipes.aCategory[pShop->nCategory], &nCount);
        break;
    case TAB_REWARDS:
        apRows = func_ov026_0208de40(rewards.aList[pShop->nCategory], &nCount);
        break;
    }
    if (apRows != 0 || bForce != 0) {
        ...
        if (apRows != 0 && ctx->nTab != TAB_ITEMS) {
            func_ov026_0208e218(apRows, pView->nRow, 1);
        }
        func_ov026_020874b4();
        func_ov026_0208737c(nCount);
    }
    return apRows != 0;
}
