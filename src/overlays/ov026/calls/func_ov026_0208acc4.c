/* func_ov026_0208acc4 -- Ov008_ShopTabSelectTick: one frame of the shop's
...
extern int   func_ov026_02085b9c(Ov008ChoiceBox *pBox);           /* Ov008_CursorInBox */
extern void  func_ov026_02086504(void);                           /* Ov008_RefreshTabCursor */
...
extern int   func_ov026_0208afc4(void);                           /* Ov008_FadeToBlackStep */
extern void *func_ov026_0208b028(void);                           /* enter the tab */

void *func_ov026_0208acc4(void)
{
    void *pNext;
    ...
    if (nButtons & BTN_UP) {
        ...
        if (ctx->nTabCursor == ENTRY_EXIT) {
            ...
            pNext = func_ov026_0208afc4;
        } else {
            func_02033b78(0, SOUND_SELECT);
            pNext = func_ov026_0208b028;
        }
        ctx->nStep = 0;
    ...
    if (pNext == 0 && ctx->bTouch != 0) {
        order = *(Ov008TabOrder *)data_ov026_020910f0;
        ...
        for (i = nFirst; i < nLast; i++) {
            if (func_ov026_02085b9c(&data_ov026_02091134[i]) != 0) {
                nCursor = pRow[i];
