/* func_ov026_0208dd34 -- Ov008_BuildRecipeList: collect the recipes listed under
...
extern int func_ov026_0208dc68(int nCategory, int nIndex);             /* Ov008_IsRecipeListed */
extern void NNSi_FndFreeFromDefaultHeap(void *pBlock);
extern void *NNSi_FndAllocFromDefaultExpHeap(u32 nSize);              /* AllocDefault */

Ov008RecipeRecord **func_ov026_0208dd34(int nCategory, int *pCount)
{
    int nListed = 0;
    u32 i;
    int nOut;

    for (i = 0; i < data_ov026_0209136c->nRecipeCount; i++) {
        if (func_ov026_0208dc68(nCategory, i) != 0) {
            nListed++;
        }
    }
    if (nListed == 0) {
    ...
    data_ov026_0209136c->apListed = NNSi_FndAllocFromDefaultExpHeap(nListed * sizeof(Ov008RecipeRecord *));
    nOut = 0;
    for (i = 0; i < data_ov026_0209136c->nRecipeCount; i++) {
        if (func_ov026_0208dc68(nCategory, i) != 0) {
            data_ov026_0209136c->apListed[nOut] = &data_ov026_0209136c->pRecipes[i];
            nOut++;
        }
    }
    *pCount = nOut;
    return data_ov026_0209136c->apListed;
}
