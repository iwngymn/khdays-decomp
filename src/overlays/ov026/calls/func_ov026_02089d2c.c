/* func_ov026_02089d2c -- Ov008_OpenShopDetailPanel: open the shop's detail
...
extern int   func_ov026_0208e6a0(Ov008ParamRecord *pRecord);               /* Ov008_IsEntryVisible */
...
void func_ov026_02089d2c(void)
{
    ...
    hSlots = ctx->hSlots;
    pPanel->nSelection = func_ov026_0208e6a0(ctx->detail.pRecord) != 0 ? 0 : 2;
    func_ov026_020835c4(ctx->tracker, func_ov026_02083530(ctx->tracker, TAG_DETAIL_OPEN));
    for (i = 0; i < CHOICE_BOXES; i++) {
        ...
        ctx->aChoiceCell[i][0] = func_ov026_02085d7c(hSlots, 1, 0, pos.nX, pos.nY);
        ctx->aChoiceCell[i][1] = func_ov026_02085d7c(hSlots, 2, 0, pos.nX, pos.nY);
    }
    pos.nX = 0x80000;
    func_ov026_02084330(pWidgets, func_ov026_0208427c(pWidgets, WIDGET_CURSOR), &pos);
    func_ov026_020843e8(pWidgets, func_ov026_0208427c(pWidgets, WIDGET_CURSOR), pPanel->nSelection == 0);
    pPanel->pTextLeft = func_ov026_02084d74(ctx->textLoader, 0x14);
    pPanel->pTextRight = func_ov026_02084d74(ctx->textLoader, 0x15);
    pPanel->apLabel[0] = func_ov026_02084d74(ctx->textLoader, 0x13);
    pPanel->apLabel[1] = func_ov026_02084d74(ctx->textLoader, 0x16);
    pPanel->apLabel[2] = func_ov026_02084d74(ctx->textLoader, 0x23);
    pPanel->nTitleWidth = func_020303bc(ctx->detailSurface, pPanel->pRecord->pItemDef->pName);
    pos.nY = 0x40000;
    pos.nX = (0x87 - ((pPanel->nTitleWidth + 0x10) >> 1)) << 12;
    pPanel->hTitleCell = func_ov026_02085d7c(hSlots, 0, ctx->nCell, pos.nX, pos.nY);
    pPanel->hIconCell = func_ov026_02085d7c(hSlots, 0xb, 1, pos.nX - 0xb000, pos.nY + 0x4000);
    if (pPanel->pRecord->pItemDef->nIconId != 0) {
        func_02032710(hSlots, pPanel->hTitleCell, 1);
        func_02032710(hSlots, pPanel->hIconCell, data_0204be18->aItemCount[pPanel->pRecord->pItemDef->nItemId] != func_ov026_0208df80(pPanel->pRecord));
