/* func_ov026_020855c4 -- Ov008_InitPanelSlotManagers: create the panel
...
extern void func_ov026_02083e30(void *pManager, Ov008SlotManagerCfg *pCfg, int nArg, int nSlots); /* InitSubsystemObject */
extern u32  func_0203243c(void *hSlots, u32 nHandle);              /* ForwardTo_02031d90 */
extern void func_ov026_020841bc(void *pManager, u8 *pResource, int nCount); /* Ov008_LoadBlockProcessAndFree */

void func_ov026_020855c4(void)
{
    ...
    cfg.f10 = 0;
    cfg.f14 = 0;
    func_ov026_02083e30(ctx->managerB, &cfg, 0, 1);
    ctx->hSlotsB = ctx->managerB;
    nSlot = data_ov026_0209114b[ctx->nPage * 4];
    if (nSlot >= 0) {
        func_0203243c(ctx->hSlotsB, CELL_HANDLE(ctx->pContainerB, nSlot & SLOT_MASK));
    }
    func_ov026_020841bc(ctx->managerB, data_ov026_02091288, 0xf);
    ...
    cfg.f10 = 0;
    cfg.f14 = 0;
    func_ov026_02083e30(ctx->managerA, &cfg, 0, 2);
    ctx->hSlots = ctx->managerA;
    nSlot = data_ov026_02091149[ctx->nPage * 4];
    if (nSlot >= 0) {
        func_0203243c(ctx->hSlots, CELL_HANDLE(ctx->pContainerB, nSlot & SLOT_MASK));
    }
    func_ov026_020841bc(ctx->managerA, data_ov026_0209129c, 0x1b);
    nSlot = data_ov026_02091148[ctx->nPage * 4];
    if (nSlot >= 0) {
        func_0203243c(ctx->hSlots, CELL_HANDLE(ctx->pContainerB, nSlot & SLOT_MASK));
    } else {
