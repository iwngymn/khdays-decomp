/* func_ov026_0208a2fc -- Ov008_OpenBuyDialog: open the shop's buy dialog
...
extern void  func_ov026_020835c4(void *pTracker, void *pCell);              /* Ov008_TagTracker_InvokeCallback */
...
extern void  func_ov026_02085ddc(int nCaption);                             /* set the dialog caption */
extern int   func_ov026_02085d7c(int hSlots, int nKind, int nSlot, int nY, int nX); /* create a cell */
extern void  func_ov026_02086910(void);                                     /* Ov008_ShowCounterPanel */
extern void  func_ov026_02086ebc(void);                                     /* refresh the totals */
...
extern void  func_ov026_02085e58(u16 *pText, u32 nIcon);                    /* Ov008_DrawDescriptionText */
extern u16  *func_ov026_02090ed4(int *pList, Ov008ItemDef *pItemDef);       /* text of an item def */
extern void  func_ov026_02086820(int hSlots, u32 nValue, int *aCell);       /* Ov008_ShowThreeDigitCells */
extern void  func_02033b78(int nKind, int nSound);                          /* PlaySound */

void func_ov026_0208a2fc(void)
{
    ...
    if (pDialog->bFirstOpen != 0) {
        ...
        func_ov026_020835c4(ctx->trackerB, func_ov026_02083530(ctx->trackerB, TAG_BUY_SECONDARY));
        func_ov026_020835c4(ctx->tracker, func_ov026_02083530(ctx->tracker, TAG_BUY_PRIMARY));
        func_ov026_020843e8(pWidgets, func_ov026_0208427c(pWidgets, 1), 0);
        func_ov026_020843e8(pWidgets, func_ov026_0208427c(pWidgets, 9), 1);
        func_ov026_020843e8(pWidgets, func_ov026_0208427c(pWidgets, 7), 1);
        func_ov026_02085ddc(0x17);
        for (i = 0; i < DIGIT_CELLS; i++) {
            pDialog->aDigitCell[i] = func_ov026_02085d7c(hSlots, 0x15, 0, (0xc0 + i * 8) << 12, 0x52000);
        }
        pDialog->aCategoryCell[0] = func_ov026_02085d7c(hSlots, 0x13, 0, 0xb0000, 0x50000);
        pDialog->aCategoryCell[1] = func_ov026_02085d7c(hSlots, 0x14, 0, 0xb0000, 0x50000);
        func_ov026_02086910();
        func_ov026_02086ebc();
        pDialog->bFirstOpen = 0;
    }
    func_0203011c(pSurface, 0x20, 0x30, 0xc0, 0x60);
    func_ov026_02085bf4(pSurface, pRecord->pName, 0x80, 0x3b, TEXT_STYLE, 0x10);
    func_ov026_02085bf4(pSurface, func_ov026_02084d74(ctx->textLoader, 0x19), 0x28, 0x4c, TEXT_STYLE, 8);
    func_02032710(hSlots, pDialog->aCategoryCell[0], 0);
    func_02032710(hSlots, pDialog->aCategoryCell[1], 0);
    if (pRecord->pItemDef == 0) {
        func_ov026_02085e58(pRecord->pRecipeText, ICON_NONE);
    } else {
        pText = func_ov026_02090ed4(ctx->textList, pRecord->pItemDef);
        func_ov026_02085e58(pText, pRecord->pItemDef->nIcon);
    }
    func_ov026_02086820(hSlots, pRecord->nPrice, pDialog->aDigitCell);
