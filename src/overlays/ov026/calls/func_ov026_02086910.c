/* func_ov026_02086910 -- Ov008_ShowCounterPanel: build the counter panel
...
extern int   func_ov026_02085d7c(int *hSlots, unsigned int nKind, int nSlot, int nY, int nX); /* create a cell */
...
void func_ov026_02086910(void)
{
    ...
    if (ctx->nTab != TAB_SPECIAL) {
        for (i = 0; i < DIGIT_COUNT; i++) {
            pPanel->aDigitA[i] = func_ov026_02085d7c(hSlots, 0, 0, (0x9f + i * DIGIT_STEP) << 12, CELL_X);
            pPanel->aDigitB[i] = func_ov026_02085d7c(hSlots, 2, 0, (0xe2 + i * DIGIT_STEP) << 12, CELL_X);
            pPanel->aDigitC[i] = func_ov026_02085d7c(hSlots, 3, 0, (0xe2 + i * DIGIT_STEP) << 12, CELL_X);
        }
        pPanel->hExtra = func_ov026_02085d7c(hSlots, 4, 0, EXTRA_Y, CELL_X);
        ...
        func_ov026_020843e8(pWidgets, func_ov026_0208427c(pWidgets, 0x1e), 0);
        func_ov026_020843e8(pWidgets, func_ov026_0208427c(pWidgets, 0x1f), 0);
        for (i = 0; i < DIGIT_COUNT; i++) {
            pPanel->aDigitA[i] = func_ov026_02085d7c(hSlots, 0, 0, (0x8c + i * DIGIT_STEP) << 12, CELL_X_TAB3);
            pPanel->aDigitB[i] = func_ov026_02085d7c(hSlots, 0, 0, (0xc4 + i * DIGIT_STEP) << 12, CELL_X_TAB3);
        }
    }
    pPanel->bReady = 0;
    pPanel->bVisible = 1;
