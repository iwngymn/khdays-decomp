/* func_ov224_020d0d38 -- the move CHOOSER for ov221's family: pick the next move and re-arm the
 * think timer. Returns 1 if a move ended up queued at ctx[0]+0x1c7, 0 if not.
 *
 * One of a 4-member shape family; the twins live in ov221/ov222/ov226 and are byte-identical
...
int func_ov224_020d0d38(int self, int dist) {
    int *ctx;
    int roll;
    int charged;
    int clear;
