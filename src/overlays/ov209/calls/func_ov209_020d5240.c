/*
 * func_ov209_020d5240 -- x3. AI-state entry: reconfigure flags, spawn effect 0x4d, dispatch.
 * hw60.hi &= ~1 then hw60.hi |= 0x86 on the +0x60 flags; set bits 0x3 in the halfword at
 * state[0]+0x1ae; clear bit0 of the low byte of the words at *(state[0]+0x3b8)+8 and *(state[0]+0x3b4)+8.
 * Spawn via 020c5af8(state[0], 0, 0x4d, state[3]). Clear *(u8)(state[0]+0x1c7), hand off 0203c634 (cb=0).
 */
...
void func_ov209_020d5240(int *self) {
    ...
    *hw = h & ~0xff00 | (((((unsigned int)h << 0x10) >> 0x18 | 0x86) << 0x18) >> 0x10);
    ...
    func_ov107_020c5af8(*state, 0, 0x4d, state[3]);
