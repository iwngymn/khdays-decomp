/* func_ov263_020d199c -- AI hover-dive step (byte-identical in ov232 /
...
extern const Vec3 data_ov263_020d36b0;                                 /* base heading */
extern const Vec3 data_02042258;                                       /* kVecForward */
extern void  func_ov263_020d06bc(Vec3 *pOut, DiveNode *pNode, const Vec3 *pIn); /* rotate by the owner yaw */
extern void  func_ov263_020d0628(DiveNode *pNode);                      /* update the heading */
...
extern int   func_ov263_020d0750(DiveNode *pNode, const Vec3 *pDir, int nArg); /* probe for a hit */
...
void func_ov263_020d199c(DiveNode *pNode)
{
    Vec3 vDir;
    Vec3 vAxis;
    Vec3 vForward;
    ...
    bDone = 0;
    func_ov263_020d06bc(&vDir, pNode, &vDir);
    pState->vVel = vDir;
    pState->nTimer += pNode->pClock[0x2c / 4];
    switch (pState->nPhase) {
    case 0:
        func_ov263_020d0628(pNode);
    ...
    if (func_ov263_020d0750(pNode, &vDir, 0) != 0) {
        pState->nHits++;
        if (pState->nHits == 1) {
