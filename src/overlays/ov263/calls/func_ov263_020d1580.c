/* func_ov263_020d1580 -- the hover tick: track the reference point, nudge it by speed, and pick an
 * exit.
 *
 * The tracked point (func_ov263_020d06bc against *(ctx[0]+0x388)+0x2c) lands at ctx+0x30, then a
 * pure-Z offset is added on top, keyed off the speed at +0x20:
 *   under 0x800 -> -0x100 (pull in) · over 0xc000 -> +0x200 (push out) · between -> 0.
 * That offset goes through func_ov263_020d06bc as well (in-place: out and ref are the same
 ...
 *     0xb;
 *   - with both +0x24 and +0x58 idle, func_ov263_020d0c00 gets a say and can end it outright;
 ...
 * (the address escapes into func_ov263_020d06bc, so the stores are not dead). */
...
void func_ov263_020d1580(int self) {
    ...
    func_ov263_020d06bc(&pt, self, (const VecFx32 *)(*(int *)(ctx[0] + 0x388) + 0x2c));
    *(VecFx32 *)((char *)ctx + 0x30) = pt;
    ...
    offset.z = z;
    func_ov263_020d06bc(&offset, self, &offset);
