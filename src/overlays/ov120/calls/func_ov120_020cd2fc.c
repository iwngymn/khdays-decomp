/*
 * func_ov120_020cd2fc -- Ov120_AreaAttack_Broadcast.
 *
 * One frame of the area attack that follows the charge. Shared byte-for-byte with ov121 and
 * ov122; only the call targets differ.
 *
 ...
 *
 * Ghidra: Ov120_AreaAttack_Broadcast(Ov120ActionNode *pNode), types /khdays/Ov120ActionNode,
 ...
 *    020cdef0 and 020cdef4. Declared as two separate locals the four-byte opener lands at sp+8
...
extern struct Ov120AreaOpener data_ov120_020cdef0;
extern struct Vec3 data_02042258;
extern struct Ov120AreaMsg data_ov120_020cdef4;
...
void func_ov120_020cd2fc(struct Ov120ActionNode *node)
{
    int results[4];
    ...
    if (state->bOneShot4c == 0 && state->nElapsed40 >= 0x555) {
        pair.aOpener = data_ov120_020cdef0;
    ...
    if (state->nElapsed40 >= 0x555) {
        ...
        if (count > 0) {
            fwd = data_02042258;
            pair.aTemplate = data_ov120_020cdef4;
            do {
