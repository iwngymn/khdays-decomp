/* func_ov160_020ce474 -- AI drop step: the state's hit target (+0x2c) is
...
extern int   func_ov160_020ce02c(DropState *pState, Vec3 *pStep, Vec3 *pNormal, fx32 nY);
...
extern void  func_ov160_020ce738(void);
...
void func_ov160_020ce474(DropNode *pNode)
{
    ...
    vStep.y = -0x32000;
    if (func_ov160_020ce02c(pState, &vStep, &vNormal, vStep.y) != 0) {
    ...
    if (func_ov160_020ce02c(pState, &vStep, &vNormal, vStep.y) != 0) {
        ...
        func_0203c634(pNode, pNode->nSlot, func_ov160_020ce738);
    } else {
