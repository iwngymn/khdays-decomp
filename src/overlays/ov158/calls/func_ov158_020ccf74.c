/*
 * func_ov158_020ccf74 -- x3. AI-state entry: reconfigure flags, spawn effect 0x48, dispatch.
 * hw60.hi |= 0x80 then hw60.hi &= ~0xc on the +0x60 flags; set bit0 of the halfword at state[0]+0x1ae;
 * clear bit0 of the low byte of the word at *(state[0]+0x388)+8; zero state[0xf]. Spawn via
...
extern void func_ov158_020cd03c(void);

void func_ov158_020ccf74(int *self) {
    ...
    *hw = h & ~0xff00 | (((((unsigned int)h << 0x10) >> 0x18 | 0x80) << 0x18) >> 0x10);
    ...
    func_0203c634((int)self, *(signed char *)((int)self + 0x20), (int)&func_ov158_020cd03c);
}
