/* func_ov138_020d2008 -- AI drop step: the state's hit target (+0x2c) is
...
extern int   func_ov138_020d1bdc(DropState *pState, Vec3 *pStep, Vec3 *pNormal, fx32 nY);
...
extern void  func_ov138_020d22cc(void);
...
void func_ov138_020d2008(DropNode *pNode)
{
    ...
    vStep.y = -0x32000;
    if (func_ov138_020d1bdc(pState, &vStep, &vNormal, vStep.y) != 0) {
    ...
    if (func_ov138_020d1bdc(pState, &vStep, &vNormal, vStep.y) != 0) {
        ...
        func_0203c634(pNode, pNode->nSlot, func_ov138_020d22cc);
    } else {
