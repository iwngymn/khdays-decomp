/* func_ov265_020d1990 -- AI hover-dive step (byte-identical in ov232 /
...
extern const Vec3 data_ov265_020d36a4;                                 /* base heading */
extern const Vec3 data_02042258;                                       /* kVecForward */
extern void  func_ov265_020d06bc(Vec3 *pOut, DiveNode *pNode, const Vec3 *pIn); /* rotate by the owner yaw */
extern void  func_ov265_020d0628(DiveNode *pNode);                      /* update the heading */
...
extern int   func_ov265_020d0750(DiveNode *pNode, const Vec3 *pDir, int nArg); /* probe for a hit */
...
void func_ov265_020d1990(DiveNode *pNode)
{
    Vec3 vDir;
    Vec3 vAxis;
    Vec3 vForward;
    ...
    pState = pNode->pState;
    vDir = data_ov265_020d36a4;
    bDone = 0;
    func_ov265_020d06bc(&vDir, pNode, &vDir);
    pState->vVel = vDir;
    pState->nTimer += pNode->pClock[0x2c / 4];
    switch (pState->nPhase) {
    case 0:
        func_ov265_020d0628(pNode);
    ...
    if (func_ov265_020d0750(pNode, &vDir, 0) != 0) {
        pState->nHits++;
        if (pState->nHits == 1) {
