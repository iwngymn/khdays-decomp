/* func_ov231_020cdd3c -- AI hover-dive step (byte-identical in ov232 /
...
extern const Vec3 data_ov231_020cfa4c;                                 /* base heading */
extern const Vec3 data_02042258;                                       /* kVecForward */
extern void  func_ov231_020cca74(Vec3 *pOut, DiveNode *pNode, const Vec3 *pIn); /* rotate by the owner yaw */
extern void  func_ov231_020cc9e0(DiveNode *pNode);                      /* update the heading */
...
extern int   func_ov231_020ccb08(DiveNode *pNode, const Vec3 *pDir, int nArg); /* probe for a hit */
...
void func_ov231_020cdd3c(DiveNode *pNode)
{
    Vec3 vDir;
    Vec3 vAxis;
    Vec3 vForward;
    ...
    pState = pNode->pState;
    vDir = data_ov231_020cfa4c;
    bDone = 0;
    func_ov231_020cca74(&vDir, pNode, &vDir);
    pState->vVel = vDir;
    pState->nTimer += pNode->pClock[0x2c / 4];
    switch (pState->nPhase) {
    case 0:
        func_ov231_020cc9e0(pNode);
    ...
    if (func_ov231_020ccb08(pNode, &vDir, 0) != 0) {
        pState->nHits++;
        if (pState->nHits == 1) {
