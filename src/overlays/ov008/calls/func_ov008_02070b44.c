/* func_ov008_02070b44 -- Ov008_LayoutMissionDots: lay out the mission list's
...
extern void func_ov008_0206fe98(int nOwner, u32 nTag, short nX, short nY); /* Ov008_SetActiveTagValueAndFlag2 */
...
void func_ov008_02070b44(Ov008MissionList *pList)
{
    int i;
    int nX;
    int nOpen;
    ...
    func_ov008_02055aec(nOwner, func_ov008_02055a8c(nOwner, TAG_DOT_ACTIVE), 0);
    nOpen = -1;
    ...
    for (i = 0; i < pList->nDots; i++, nX += DOT_STEP) {
        if (i < pList->nDotsDone) {
            func_ov008_0206fe50(nOwner, TAG_DOT_DONE, nX, DOT_Y);
        } else if (i < nOpen) {
            func_ov008_0206fe98(nOwner, TAG_DOT_ACTIVE, nX, DOT_Y);
        } else {
            func_ov008_0206fe50(nOwner, TAG_DOT_TODO, nX, DOT_Y);
        }
    }
}
