/* func_ov008_0205e2e0 -- Ov008_RefreshInventoryRows: redraw the eight
...
extern void  func_ov008_0205e058(Ov008MenuContext *pCtx, int nRow, void *pText, int nColour); /* Ov008_UploadTileSlot */
extern void  func_ov008_0205e0f4(Ov008MenuContext *pCtx, int nRow, int nPlaced, int nOwned); /* draw the counts */
...
void func_ov008_0205e2e0(Ov008MenuContext *pCtx, int nArg, int nScroll)
{
    int nRow;
    ...
    if (pItem != 0) {
        do {
            ...
            func_ov008_02056988(pCtx->aRowCell[nRow].aTexParams, func_ov008_0205da50(pCtx, pItem->pRecord->nTag)->pTexture);
            pRecord = pItem->pRecord;
            if (pRecord->nCategory == CATEGORY_FIXED) {
                func_ov008_0205e058(pCtx, nRow, pRecord->pName, COLOUR_DONE);
                func_ov008_0205e0f4(pCtx, nRow, -1, pItem->nCount);
                ...
                if (pItem->nPlaced >= pItem->nCount) {
                    ...
                    func_ov008_0205e058(pCtx, nRow, pRecord->pName, COLOUR_ACTIVE);
                    pCtx->aRowCell[nRow].nState = 0;
                }
                func_ov008_0205e0f4(pCtx, nRow, pItem->nPlaced, pItem->nCount);
            }
            if (func_02023588(pItem->pRecord->nItemId + FLAG_ITEM_HIDDEN) != 0) {
                func_ov008_02054ba4(nCtx, func_ov008_02054788(nCtx, nRow + WIDGET_ROW_BASE), 0);
            } else {
                pEntry = func_ov008_02054788(nCtx, nRow + WIDGET_ROW_BASE);
                func_ov008_02054c80(nCtx, pEntry, (u16)(func_ov008_0205db98(pCtx, pItem->pRecord->nItemId) != 0));
                func_ov008_02054ba4(nCtx, func_ov008_02054788(nCtx, nRow + WIDGET_ROW_BASE), 1);
            }
            if (pItem->nPlaced != 0) {
                func_ov008_02054ba4(nCtx, func_ov008_02054788(nCtx, nRow + WIDGET_MARK_BASE), 1);
            } else {
                func_ov008_02054ba4(nCtx, func_ov008_02054788(nCtx, nRow + WIDGET_MARK_BASE), 0);
            }
            nRow++;
    ...
    for (; nRow < ROW_COUNT; nRow++) {
        func_ov008_0205e058(pCtx, nRow, 0, COLOUR_ACTIVE);
    ...
    func_ov008_02054ba4(nCtx, func_ov008_02054788(nCtx, WIDGET_ARROW_UP), bVisible);
    if (nScroll + ROW_COUNT < pCtx->nListCount) {
        bVisible = 1;
    } else {
        bVisible = 0;
    }
    func_ov008_02054ba4(nCtx, func_ov008_02054788(nCtx, WIDGET_ARROW_DOWN), bVisible);
}
