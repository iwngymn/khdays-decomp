/* func_ov008_02069570 -- Ov008_DrawSaveSlotTexts: draw the text of the three
 ...
 * data_ov008_020904f8) at +0x89 / +0x92 / +0xa0, and the separator (record
...
extern const u16 data_ov008_020904f8[];                                  /* "%d"-style number format */
...
extern void  func_ov008_020678d4(u32 nTime, u16 *pHours, u8 *pMinutes, u8 *pSeconds); /* ov008_SplitTimeUnitsHMS */
extern void  func_020262a0(u16 *pDst, int nCap, const u16 *pFormat, ...); /* Text_FormatUtf16 */

void func_ov008_02069570(Ov008SaveMenu *pMenu)
{
    u16 aText[TEXT_CAP];
    u16 nHours;
    u8  nSeconds;
    ...
    for (nSlot = 0; nSlot < SLOT_COUNT; nSlot++, pEntry++) {
        ...
        if (pEntry->nMode == MODE_EMPTY) {
            func_ov008_02068120(pMenu->slotTextSurface, nX + 0x37, nY + 0xc, 4, func_ov008_02055c84(pMenu->varRecords, 0xc), 0);
        } else if (pEntry->nMode == MODE_SAVED) {
            func_ov008_020678d4(pEntry->nPlayTime, &nHours, &nMinutes, &nSeconds);
            for (i = 0; i < 7; i++) {
                nCol = SLOT_COL_BASE;
                switch (i) {
                case 0:
                    func_020262a0(aText, TEXT_CAP, func_ov008_02055c84(pMenu->varRecords, 9), pEntry->nDay);
                    nCol += 2;
                    nShadow = 0;
                    break;
                case 1:
                    func_020262a0(aText, TEXT_CAP, func_ov008_02055c84(pMenu->varRecords, 4), pEntry->nMunny);
                    nCol += 0x70;
                    nShadow = 1;
                    break;
                case 2:
                    nCol += 0x89;
                    func_020262a0(aText, TEXT_CAP, data_ov008_020904f8, nHours);
                    nShadow = 1;
                    break;
                case 3:
                    nCol += 0x92;
                    func_020262a0(aText, TEXT_CAP, data_ov008_020904f8, nMinutes);
                    nShadow = 2;
                    break;
                case 4:
                    nCol += 0xa0;
                    func_020262a0(aText, TEXT_CAP, data_ov008_020904f8, nSeconds);
                    nShadow = 2;
                    break;
                case 5:
                case 6:
                    ...
                    func_020262a0(aText, TEXT_CAP, func_ov008_02055c84(pMenu->varRecords, 8));
                    nCol += nOffset;
                    nShadow = 1;
                    break;
                }
                func_ov008_02068120(pMenu->slotTextSurface, nX + nCol, nY + 0x17, 2, aText, nShadow);
            }
        }
    }
}
