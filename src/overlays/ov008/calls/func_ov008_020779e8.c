/* func_ov008_020779e8 -- Ov008_MissionMenuStep: move the mission menu's cursor
...
void func_ov008_020779e8(Ov008MissionMenu *pMenu, int nStep)
{
    ...
    for (i = 0; i < pMenu->nCount; i++) {
        ...
        pEntry = func_ov008_0205b670(nIndex);
        if (pEntry == 0) {
            bOk = 0;
        }
        if (bModal == 0) {
        ...
        if (bOk) {
            ...
            func_ov008_020511c8(pEntry->missionId);
            break;
        }
        nIndex += nStep;
    }
    func_ov008_0205b4ec(pEntry->nTextSlot, 0);
    func_ov008_02076158(pMenu);
    func_ov008_02076780(pMenu, pMenu->nPage);
}
