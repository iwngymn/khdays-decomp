/* func_ov008_020676b4 -- Ov008_PollSaveCardOp: poll the save menu's pending card
...
extern int func_ov008_0208bd10(void);                          /* Ov008_PollSaveLoad */
extern int func_ov008_0208be4c(void);                          /* Ov008_CardTransferStep */
extern void func_020235a8(int nFlag);                          /* GameState_SetFlag */
extern void func_ov008_0208bae4(int bArm);                     /* re-arm the card poll */

int func_ov008_020676b4(Ov008SaveMenu *pMenu, int nSlot, int nPhase, int nArg)
{
    int nResult;

    switch (pMenu->nCardOp) {
    case CARD_OP_LOAD:
        nResult = func_ov008_0208bd10();
        break;
    case CARD_OP_IDLE:
        nResult = 0;
        break;
    case CARD_OP_TRANSFER:
        nResult = func_ov008_0208be4c();
        break;
    default:
