/* func_ov008_02063cac -- Ov008_FindSpareSingleForNode: pick a panel item
...
Ov008Message15Record *func_ov008_02063cac(Ov008MenuContext *pCtx, Ov008TrackedNode *pNode)
{
    int nRow;
    int nCol;
    ...
    for (nRow = pNode->nTop; nRow < pNode->nTop + pNode->nHeight; nRow++) {
        for (nCol = pNode->nLeft; nCol < pNode->nLeft + pNode->nWidth; nCol++) {
            ...
            if ((u32)func_ov008_0205e734(pCtx, pRecord->nItemId) < data_0204be18->aItemCount[pRecord->nItemId]) {
                return pRecord;
            }
        }
    }
    ...
    for (nRow = 1; nRow < ITEM_ID_END; nRow++) {
        ...
        if (pItem->nCategory == CATEGORY_SINGLE && pItem->nPlacedSlot < 0
            && (u32)func_ov008_0205e734(pCtx, pItem->nItemId) < data_0204be18->aItemCount[pItem->nItemId]) {
            return pItem;
        }
    }
    return 0;
