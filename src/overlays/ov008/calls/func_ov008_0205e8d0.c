/* func_ov008_0205e8d0 -- Ov008_BuildInventoryLists: fill the eight category
 * lists (+0x304, 12 bytes each) of the grid inventory.  The per-category
 * filter table data_ov008_0208f208 (8 ints, 7 = any category) is copied to the
...
extern const Ov008CategoryFilter data_ov008_0208f208;
extern GameState *data_0204be18;
extern int   func_ov008_0205ccd8(int nItemId);                          /* Ov008_IsGridItemEligible */
...
extern void  func_ov008_0205e868(Ov008MenuContext *pCtx, NNSFndList *pList, Ov008InventoryItem *pItem); /* append */

void func_ov008_0205e8d0(Ov008MenuContext *pCtx)
{
    ...
    filter = data_ov008_0208f208;
    for (i = 0; i < CATEGORY_COUNT; i++) {
        pCtx->nTotal = 0;
        for (nItemId = 1; nItemId < ITEM_ID_END; nItemId++) {
            if (func_ov008_0205ccd8(nItemId) != 0 && data_0204be18->aItemCount[nItemId] != 0) {
                ...
                if (filter.aCategory[i] == CATEGORY_ANY || pRecord->nCategory == filter.aCategory[i]) {
                    ...
                    pItem->nPlaced = func_ov008_0205e734(pCtx, pItem->pRecord->nItemId);
                    pCtx->aPlaced[pItem->pRecord->nItemId] = pItem->nPlaced;
                    func_ov008_0205e868(pCtx, &pCtx->aList[i], pItem);
                    pCtx->aCount[i]++;
                }
            }
        }
