/* func_ov008_020604e4 -- Ov008_PlaceNodeShape: place tracked node pNode with
...
extern int   func_ov008_02060484(Ov008MenuContext *pCtx);               /* Ov008_FindFreeSlotId */
...
extern void  func_ov008_020548bc(int nCtx, void *pEntry, Ov008CellOffset *pOffset); /* Ov008_ApplyOffsetSum */
...
{
    pCtx->pCountText = func_ov008_02055c84(pCtx->records, TEXT_COUNT);
}

int func_ov008_020604e4(Ov008MenuContext *pCtx, Ov008TrackedNode *pNode, int nPage, int nLeft, int nTop, int bDryRun)
{
    int nWidth;
    int nHeight;
    int bDone;
    ...
    if (nLeft >= 0 && nTop >= 0) {
        ...
        if (bDryRun == 0 && pCtx->nVisiblePage == nPage) {
            nSlotId = func_ov008_02060484(pCtx);
            if (nSlotId >= 0) {
                nCtx = func_ov008_02050c54();
                pEntry = func_ov008_02054788(nCtx, nSlotId);
                func_ov008_02054b18(nCtx, pEntry, pNode->nSubitemSet);
                offset.nX = nLeft << 16;
                offset.nY = nTop << 16;
                func_ov008_020548bc(nCtx, pEntry, &offset);
                func_ov008_02054c80(nCtx, pEntry, (u16)pNode->pEntry->nFrame);
                func_ov008_02054ba4(nCtx, pEntry, 1);
