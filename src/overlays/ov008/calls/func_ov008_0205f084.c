/* func_ov008_0205f084 -- Ov008_HighlightListRow: move the inventory list
...
extern int  func_ov008_0208e66c(int *pList, Ov008Message15Record *pRecord); /* text index of a record */
...
extern void func_ov008_0205f050(Ov008MenuContext *pCtx, int nRow, int bOn); /* draw / clear a row highlight */
...
void func_ov008_0205f084(Ov008MenuContext *pCtx, int nRow)
{
    UiLayoutPos pos = { 0, 0 };
    int nCtx;
    ...
    if (pCtx->menuState == STATE_DRAG) {
        pos.nY = nRow << 16;
        func_ov008_020548bc(nCtx, func_ov008_02054788(nCtx, WIDGET_DRAG), &pos);
    }
    pItem = func_02010154(pCtx->pList, (u16)(pCtx->nScrollRow + nRow));
    if (pItem != 0) {
        func_ov008_0205e7ec(pCtx, pItem->pRecord->nItemId);
        if (pCtx->menuState == STATE_DRAG) {
            func_ov008_0205eafc(pCtx, 0, func_ov008_0208e66c(pCtx->textList, pItem->pRecord), COLOUR_ACTIVE);
            pIcon = func_ov008_02054788(nCtx, WIDGET_ICON);
            if (pItem->pRecord->nShapeIndex >= 0) {
                func_ov008_02054ba4(nCtx, pIcon, 1);
                func_ov008_02054c80(nCtx, pIcon, (u16)pItem->pRecord->nShapeIndex);
            } else {
    ...
    if (nRow >= 0 && nRow < VISIBLE_ROWS) {
        if (nRow != pCtx->nHighlightRow) {
            func_ov008_0205f050(pCtx, pCtx->nHighlightRow, 0);
        }
        if (pItem != 0) {
            if (func_02023588(pItem->pRecord->nItemId + FLAG_ITEM_HIDDEN) != 0) {
                func_ov008_02054ba4(nCtx, func_ov008_02054788(nCtx, nRow + ROW_WIDGET), 0);
            } else {
                func_ov008_02054c80(nCtx, func_ov008_02054788(nCtx, nRow + ROW_WIDGET), (u16)(func_ov008_0205db98(pCtx, pItem->pRecord->nItemId) != 0));
                func_ov008_02054ba4(nCtx, func_ov008_02054788(nCtx, nRow + ROW_WIDGET), 1);
            }
        }
        func_ov008_0205f050(pCtx, nRow, 1);
