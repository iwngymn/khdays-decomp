/* func_ov008_02061b18 -- Ov008_LiftTrackedNode: collect the cells of tracked
...
void func_ov008_02061b18(Ov008MenuContext *pCtx, Ov008TrackedNode *pNode)
{
    u32 nPage;
    int nRow;
    int nCol;
    ...
    for (nRow = 0; nRow < pNode->nHeight; nRow++) {
        for (nCol = 0; nCol < pNode->nWidth; nCol++) {
            ...
            func_ov008_02056988(pCtx->aDragCell[nLifted].aTexture, func_ov008_0205da50(pCtx, pCell->pRecord->nTag)->pParams);
            nLifted++;
        }
    }
}
