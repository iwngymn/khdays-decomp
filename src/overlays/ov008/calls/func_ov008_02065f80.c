/* func_ov008_02065f80 -- Ov008_MenuKeyUp: the menu's "up" key handler.
 ...
 * cursor one row up (func_ov008_0206245c) and, if that took, arms the move,
...
void func_ov008_02065f80(Ov008MenuContext *pCtx)
{
    u16 touch[3];
    ...
    case 0:
    case 1:
        if (func_ov008_0206245c(pCtx, pCtx->nColumn, pCtx->nRowSel - 1, 1) == 0) {
            return;
        }
        pCtx->bMoveArmed = 1;
        pCtx->nHoldCount = 0;
    ...
    case 2:
        ...
        if (func_ov008_02062288(pCtx, (int)(func_02020400(pCtx->nPage + pCtx->nPageCount - 1, pCtx->nPageCount) >> 32)) == 0) {
            return;
        }
