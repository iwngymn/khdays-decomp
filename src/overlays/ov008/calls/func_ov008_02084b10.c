/* func_ov008_02084b10 -- Ov008_DrawShopRowText: draw the texts of shop row
...
extern char  func_ov008_0208b0d0(Ov008ParamRecord *pRecord);             /* Ov008_CountSpareItemsOfChild */
extern int   func_ov008_020830c8(Ov008ParamRecord *pRecord);             /* Ov008_AreRequirementsMet */
extern void  func_ov008_02082d44(Ov008TileSurface *pSurface, void *pText, int nX, int nY, int nStyle, int nShadow); /* Ov008_DrawStringShadowed */
extern void  func_ov008_02082d98(Ov008TileSurface *pSurface, u32 nValue, int nX, int nY, int nStyle); /* Ov008_BuildTable9AndDelegate: draw a number */

void func_ov008_02084b10(int nRow, Ov008ParamRecord *pRecord)
{
    ...
    case TAB_BUY:
        nStyle = (pRecord->nPrice <= data_0204be18->nMunny && pRecord->bLevelReached == 0
            && data_0204be18->aItemCount[pRecord->pItemDef->nItemId] < func_ov008_02082234(pRecord)) ? STYLE_ON : STYLE_OFF;
        break;
    case TAB_SELL:
        nStyle = (pRecord->pItemDef->nSellable == 1 && func_ov008_0208b0d0(pRecord) > 0) ? STYLE_ON : STYLE_OFF;
        break;
    case TAB_TRADE:
        nStyle = (pRecord->nPrice <= data_0204be18->nPoints
            && data_0204be18->aItemCount[pRecord->pItemDef->nItemId] < func_ov008_02082234(pRecord)
            && func_ov008_020830c8(pRecord) != 0) ? STYLE_ON : STYLE_OFF;
        break;
    case TAB_RECIPES:
    ...
    if (ctx->nTab != TAB_RECIPES) {
        func_ov008_02082d44(pSurface, pRecord->pItemDef->pName, 0x3a, nY + 0x1b, nStyle, 8);
        if (ctx->nTab == TAB_SELL && pRecord->pItemDef->nSellable == 0) {
            func_ov008_02082d44(pSurface, data_ov008_02090fac->apVarText[3], 0xac, nY + 0x1b, STYLE_OFF, 0x20);
        } else if (ctx->nTab != TAB_BUY || pRecord->bLevelReached == 0) {
            func_ov008_02082d98(pSurface, pRecord->nPrice, 0xac, nY + 0x1b, nStyle);
        }
        ...
        if (ctx->nTab != TAB_BUY) {
            func_ov008_02082d44(pSurface, data_ov008_02090fac->apVarText[0], 0xae, nY + 0x1d, STYLE_LABEL, 8);
        } else if (ctx->nTab != TAB_BUY || pRecord->bLevelReached == 0) {
            func_ov008_02082d44(pSurface, data_ov008_02090fac->apVarText[1], 0xae, nY + 0x1d, STYLE_LABEL, 8);
        }
        pSurface->pFont = ctx->fontA;
    } else {
        bListed = ctx->nScroll + nRow < ctx->aFreeCount[ctx->nCategory];
        if (bListed) {
            func_ov008_02082d98(pSurface, pRecord->nPrice, 0xd0, nY + 0x1b, nStyle);
        } else {
            func_ov008_02082d44(pSurface, data_ov008_02090eb0, 0xd0, nY + 0x1b, nStyle, 0x20);
        }
        if (bListed && pRecord->nLevelReq == 0) {
            pText = pRecord->pRecipeText;
        } else {
            pText = data_ov008_02090eb8;
        }
        func_ov008_02082d44(pSurface, pText, 0x22, nY + 0x1b, nStyle, 8);
    }
}
