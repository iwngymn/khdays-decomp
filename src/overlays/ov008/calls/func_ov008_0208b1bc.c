/* func_ov008_0208b1bc -- Ov008_FreeParamTable: release the param table.  For
...
void func_ov008_0208b1bc(void)
{
    ...
    for (nList = 0; nList < LIST_COUNT; nList++) {
        pRecords = data_ov008_02090fb0->apRecords[nList];
        if (pRecords != 0) {
            if (nList == LIST_SHOP) {
                for (i = 0; i < data_ov008_02090fb0->aCount[nList]; i++) {
            ...
            data_ov008_02090fb0->apRecords[nList] = 0;
        }
    }
    ...
    NNSi_FndFreeFromDefaultHeap(data_ov008_02090fb0);
}
