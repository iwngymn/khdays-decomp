/* The slam's follow-through (Ghidra: Ov142_StepSlamStrike).
 *
...
struct Ov142Contact { u8 bGrounded : 1, bBlocked : 1; };
...
struct Ov142SubObj {
    ...
    char pad260[0x138];
    struct Ov142Owner *pOwner398;                                  /* 0x398 */
};
...
extern struct Ov142Cmd data_ov142_020d2630;
extern struct Ov142Cmd data_ov142_020d264c;
extern struct Ov142Cmd data_ov142_020d265a;
...
extern int func_ov107_020c8f44(struct Ov142Owner *owner, struct Ov142Query *query,
                               int *results);
extern int func_ov107_020ca918(int ent, struct Ov142SubObj *self,
                               struct Ov142Owner *owner, int mode, void *dir,
                               int flag);
extern void *func_ov107_020c9184(struct Ov142SubObj *self, struct Ov142Query *query,
                                 void **out);
extern int func_ov107_020c5cfc(void *lock, int param, struct HitCommand *req);
extern void func_ov107_020c5af8(struct Ov142Owner *owner, int a, int id, void *at);
...
void func_ov142_020d1e70(struct Ov142StepNode *node)
{
    struct Ov142StepState *state = node->pState;
    struct Ov142Query query;
    struct Ov142Quat rot;
    int results[4];
    struct Ov142Cmd cmdHit;
    void *handle;
    Fx32 hitScratchZ;
    Fx32 hitScratchY;
    Fx32 hitScratchX;
    ...
    if (state->nMode20 == 0) {
        ...
        if ((lock = func_ov107_020c9184(state->pSelf, &query, &handle)) != 0) {
            ...
            if (func_ov107_020c5cfc(lock, state->pSelf->nLockParam25c, &req) != 0) {
                struct Ov142Cmd cmdLock;

                cmdLock = data_ov142_020d264c;
                anchor = state->pAnchor;
    ...
    if (((struct Ov142Contact *)&state->pSelf->bContact17a)->bGrounded == 0
        && ((struct Ov142Contact *)&state->pSelf->bContact17a)->bBlocked == 0
        && state->nTimer24 <= 0xa000) {
        return;
    }

    {
    struct Ov142Cmd cmdEnd;

    cmdEnd = data_ov142_020d265a;
    anchor = state->pAnchor;
