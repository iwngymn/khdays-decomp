/* func_ov025_020a6a00 -- Ov008_UpdateListCursorCells: refresh the mission list's
...
extern int  func_ov025_020894b0(int nTracker, int nTag);                  /* ov008_FindEntryByTag */
extern void func_ov025_02089544(int nTracker, int nCell);                 /* Ov008_TagTracker_InvokeCallback */
extern void func_ov025_02089560(int nTracker, int nCell, int nFrame, short nY); /* Ov008_ApplyTempFieldsAndRestore */

void func_ov025_020a6a00(Ov008MissionList *pList)
{
    int nTracker;
    int nFirst;
    int nRow;
    ...
    if (func_ov025_02084e08() == 0 && pList->bTouchEnabled != 0) {
        if (pList->nSelected >= 0) {
            func_ov025_02089544(nTracker, func_ov025_020894b0(nTracker, TAG_CURSOR));
        }
        if (pList->nSelected < 0 && pList->bAnimating == 0) {
            func_ov025_02089544(nTracker, func_ov025_020894b0(nTracker, TAG_IDLE));
        }
    }
    if (pList->nWord1 >= 0) {
        func_ov025_02089560(nTracker, func_ov025_020894b0(nTracker, TAG_WORD), 2, (short)(pList->nCursorRow * 4));
    }
    nFirst = pList->nScroll / ROW_HEIGHT;
    if (pList->nSelected >= 0 && pList->bAnimating == 0) {
        nRow = pList->nSelected - nFirst;
        func_ov025_02089560(nTracker, func_ov025_020894b0(nTracker, TAG_SELECT), 2, (short)(nRow * 4));
