/* func_ov025_020ab5c8 -- Ov008_DrawMissionInfoText: draw the mission menu's
...
extern int   func_ov025_020aa168(void *pText);                               /* text width */
extern void  func_ov025_020ab01c(Ov008MissionMenu *pMenu, u16 *pMissionId);  /* draw the mission rows */
extern void  func_ov025_020ab558(Ov008MissionMenu *pMenu, int nRow);         /* draw one row caption */

void func_ov025_020ab5c8(Ov008MissionMenu *pMenu, u16 *pMissionId)
{
    void *pText;
    int nText;
    u16 i;
    ...
    if (func_ov025_02084e38() != 0) {
        if (pMenu->bTransfer == 0) {
            pText = func_ov025_02089894(pMenu->records, TEXT_TRANSFER);
            func_02030278(pMenu->surface, 3, 3, 0xf5, pText, 1);
            pMenu->nTextWidth = func_ov025_020aa168(pText);
        } else {
            switch (*pMissionId) {
            case MISSION_6C:
            ...
            pText = func_ov025_02089894(pMenu->records, nText);
            func_020302ec(pMenu->surface, 3, 3, 0xf5, 0xfb, pText);
            pMenu->nTextWidth = func_ov025_020aa168(pText);
        }
    } else {
        func_ov025_020ab01c(pMenu, pMissionId);
        pMenu->nTextWidth = 0;
        for (i = 0; i < ROW_COUNT; i++) {
            func_ov025_020ab558(pMenu, i);
        }
    }
}
