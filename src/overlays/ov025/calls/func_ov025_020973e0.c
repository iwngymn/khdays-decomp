/* func_ov025_020973e0 -- Ov008_DrawGridMenu: draw the grid menu's quads.  After
...
extern void func_ov025_0208a6e4(void);                     /* Ov008_SetupDefault3dView */
extern void func_ov025_0208a830(Ov008MenuQuad *pQuad);     /* Ov008_DrawMenuQuad */
extern void func_ov025_0208a82c(void);                     /* draw flush */

void func_ov025_020973e0(Ov008GridMenuContextView *pCtx)
{
    int i;
    int j;
    ...
    if (pCtx->cursorCell.isActive != 0) {
        func_ov025_0208a830(&pCtx->cursorCell.quad);
    }
    for (i = 0; i < COL_HEADERS; i++) {
        if (pCtx->colHeaderCells[i].isActive != 0) {
            func_ov025_0208a830(&pCtx->colHeaderCells[i].quad);
        }
    }
    for (i = 0; i < ROW_HEADERS; i++) {
        if (pCtx->rowHeaderCells[i].isActive != 0) {
            func_ov025_0208a830(&pCtx->rowHeaderCells[i].quad);
        }
    }
    for (i = 0; i < GRID_ROWS; i++) {
        for (j = 0; j < GRID_COLS; j++) {
            if (pCtx->gridDisplayCells[i][j].isActive != 0) {
                func_ov025_0208a830(&pCtx->gridDisplayCells[i][j].quad);
            }
        }
    }
    func_ov025_0208a82c();
