/* func_ov025_020947b4 -- Ov008_SelectListRow: make list row nRow the selected
...
extern int  func_ov025_02094688(Ov008MenuContext *pCtx, int nRow, int nFirst, int nTotal, int nTrackEnd, int nVisible); /* Ov008_ScrollMenuRowsTo */
...
extern void func_ov025_02090854(Ov008MenuContext *pCtx, int nRowBase, int nFirst); /* rebuild the visible rows */
...
int func_ov025_020947b4(Ov008MenuContext *pCtx, int nRow)
{
    int i;
    int nCtx;
    int nFirst;
    ...
    nFirst = func_ov025_02094688(pCtx, nRow, pCtx->nScrollRow, pCtx->nPageCount, TRACK_END, VISIBLE_ROWS);
    if (nFirst < 0) {
        return 0;
    }
    ...
    if (pCtx->nScrollRow != nFirst) {
        func_ov025_02090854(pCtx, pCtx->nRowBase, nFirst);
    } else {
        pItem = func_02010154(pCtx->pList, (u16)nFirst);
        while (pItem != 0) {
            if (func_02023588(pItem->pRecord->nItemId + FLAG_ITEM_HIDDEN) != 0) {
                func_ov025_0208884c(nCtx, func_ov025_0208843c(nCtx, i + ROW_WIDGET_A), 0);
            } else {
                func_ov025_02088928(nCtx, func_ov025_0208843c(nCtx, i + ROW_WIDGET_A), (u16)(func_ov025_0209010c(pCtx, pItem->pRecord->nItemId) != 0));
                func_ov025_0208884c(nCtx, func_ov025_0208843c(nCtx, i + ROW_WIDGET_A), 1);
            }
            if (pItem->nPlaced != 0) {
                func_ov025_0208884c(nCtx, func_ov025_0208843c(nCtx, i + ROW_WIDGET_B), 1);
            } else {
                func_ov025_0208884c(nCtx, func_ov025_0208843c(nCtx, i + ROW_WIDGET_B), 0);
            }
            i++;
            if (i >= VISIBLE_ROWS) {
                break;
    ...
    func_ov025_020915d4(pCtx, nRow - pCtx->nScrollRow);
    return 1;
}
