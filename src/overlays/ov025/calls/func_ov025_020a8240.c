/* func_ov025_020a8240 -- Ov008_RefreshMissionRows: unless the list is animating,
...
extern void func_ov025_020a54c8(Ov008MissionList *pList, int nRow, Ov008MissionListEntry *pEntry);
extern void func_020300f8(void *pSurface);                                   /* EnqueueObjGfxCommand */

void func_ov025_020a8240(Ov008MissionList *pList)
{
    int nRow = 0;
    ...
    if (pList->bAnimating == 0) {
        for (pEntry = func_ov025_0208dbbc(0); pEntry != 0; pEntry = func_ov025_0208dbbc(pEntry)) {
            func_ov025_020a54c8(pList, nRow, pEntry);
            nRow++;
            if (nRow >= ROW_COUNT) {
                break;
            }
