/* func_ov025_020925b8 -- Ov008_CanPlaceRecord: may record pRecord go to
...
extern int   func_ov025_02092538(Ov008MenuContext *pCtx);               /* Ov008_FindReactionCard */
extern int   func_ov025_0209249c(Ov008MenuContext *pCtx, int nKind);    /* Ov008_FindPanelCardOfKind */
...
int func_ov025_020925b8(Ov008MenuContext *pCtx, Ov008Message15Record *pRecord, u32 nPage, u16 nCol, u16 nRow, int bStrict)
{
    int bOk;
    u8 nId;
    ...
    if (nId != GRID_ID_LOCKED) {
        if (pRecord->nPlacedSlot >= 0) {
            if (pRecord->nItemId >= ITEM_REACTION_FIRST && pRecord->nItemId <= ITEM_REACTION_LAST) {
                if (bStrict != 0 && pCtx->bLifted == 0) {
                    bOk = func_ov025_02092538(pCtx) == 0;
                } else {
                    bOk = 1;
                }
            } else {
                if (pCtx->bLifted == 0 && bStrict != 0 && (pRecord->nCategory == 5 || pRecord->nCategory == 6)) {
                    bOk = func_ov025_0209249c(pCtx, pRecord->nKind) == 0;
                } else {
                    bOk = 1;
                }
            }
            ...
            if (nId == 0) {
                if (pRecord->nItemId >= ITEM_REACTION_FIRST && pRecord->nItemId <= ITEM_REACTION_LAST) {
                    ...
                    case 5:
                        pCtx->pCountText = func_ov025_02089894(pCtx->records, 0x1c);
                        break;
                    case 6:
                        ...
                        pCtx->pCountText = func_ov025_02089894(pCtx->records, 0x1f);
                        break;
                    case 4:
                        if (pNode != 0) {
                ...
                if (pRecord->nCategory == 6) {
                    pCtx->pCountText = func_ov025_02089894(pCtx->records, 0x1d);
                } else {
                    pCtx->pCountText = func_ov025_02089894(pCtx->records, 0x1d);
                    if (pNode == 0) {
                        return 0;
                    ...
                    if (nPage == pNode->nPage && nCol == pNode->nAnchorCol && nRow == pNode->nAnchorRow) {
                        pCtx->pCountText = func_ov025_02089894(pCtx->records, 0x21);
                    } else {
        ...
        pCtx->pCountText = func_ov025_02089894(pCtx->records, 0x22);
    }
done:
    return bOk;
}
