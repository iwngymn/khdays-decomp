/* func_ov025_020ab01c -- Ov008_BuildMissionRows: fill the mission menu's
...
void func_ov025_020ab01c(Ov008MissionMenu *pMenu, Ov008MissionListEntry *pEntry)
{
    ...
    nSeen = 0;
    nRow = 0;
    pText = func_ov025_02089894(pMenu->records, TEXT_HEADER);
    if (pText != 0) {
        pRow = &pMenu->aRow[0];
        pRow->aText[0] = 0;
    ...
    pText = func_ov025_02089894(pMenu->records, TEXT_FOOTER);
    if (nRow < ROW_COUNT) {
        pRow = &pMenu->aRow[nRow];
        pRow->aText[0] = 0;
        func_020200e4(pRow->aText, data_ov025_020b545c);
        pRow->bAvailable = 1;
        nRow++;
    }
    if (nRow < ROW_COUNT) {
        ...
        if (nItem != 0 && nItem < BONUS_ID_LIMIT) {
            ...
            if (pBonus != 0) {
                ...
                for (m = 0; m < BONUS_PAIRS; m++) {
                    ...
                    if (nSeen != 0) {
                        func_02020154(pDst->aText, data_ov025_020b5460);
                    }
