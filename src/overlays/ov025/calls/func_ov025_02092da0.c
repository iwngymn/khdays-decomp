/* func_ov025_02092da0 -- Ov008_ClearNodeCells: free every grid cell covered by a
...
void func_ov025_02092da0(Ov008MenuContext *pCtx, Ov008TrackedNode *pNode)
{
    int nRow;
    int nCol;
    int nX;
    ...
    for (nRow = 0; nRow < pNode->nHeight; nRow++) {
        for (nCol = 0; nCol < pNode->nWidth; nCol++) {
            ...
            if ((pNode->pEntry->nCellMask & (1ULL << (nCol + nRow * 8))) != 0) {
                ...
                func_ov025_02094390(pCtx, pNode->nPage, nX, nY);
            }
        }
    }
