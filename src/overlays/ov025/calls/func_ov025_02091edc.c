/* func_ov025_02091edc -- Ov008_InitGridMenuTextures: load the 3D panel icon
...
extern void  func_ov025_02091e94(Ov008GridCell *pCell, short nX, short nY, short nTile, short nPalette); /* Ov008_InitGridCell */

void func_ov025_02091edc(Ov008MenuContext *pCtx)
{
    int i;
    ...
    pCtx->pIconArchive = func_0201ef9c(data_ov025_020b4dd4, 0xe);
    ...
    for (nRow = 0; nRow < GRID_ROWS; nRow++) {
        for (nCol = 0; nCol < GRID_COLS; nCol++) {
            func_ov025_02091e94(&pCtx->gridDisplayCells[nRow][nCol], 8 + nCol * 0x10, 0x17 + nRow * 0x10, 0, PALETTE_GRID);
        }
    }
    for (i = 0; i < GRID_ROWS; i++) {
        func_ov025_02091e94(&pCtx->aRowCell[i], 0x6a, 0x18 + i * 0x10, 0, PALETTE_GRID);
    }
    for (i = 0; i < DRAG_CELLS; i++) {
        func_ov025_02091e94(&pCtx->aDragCell[i], 0, 0, 1, PALETTE_DRAG);
    }
    func_ov025_02091e94(&pCtx->currentCell, 0, 0, 1, PALETTE_DRAG);
}
