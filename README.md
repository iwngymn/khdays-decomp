...
Live counts are in **[PROGRESS.md](PROGRESS.md)**, regenerated by
`python tools/audit_progress.py`. They are not duplicated here: three files once
carried three different figures and none of them was right.

| | |
|---|---|
| Region | EU (`YKGP`) |
| Compiler | CodeWarrior `mwccarm` 3.0 build 139 | Fully-decompiled modules now include `ov002`, `ov031`, `ov107`, `ov188` and `ov189`; see `PROGRESS.md` for the complete generated list. A few precompiled-middleware translation units (e.g. the `ov028` anti-tamper crypto core) build with an older `mwccarm` via a per-file compiler override (`config/arm9/file_compilers.json`). |
